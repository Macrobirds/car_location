 ...
 */
static esp_err_t I2C_Write_Reg(unsigned char i2c_add, unsigned char reg_add, unsigned char cmd)
{
    int ret;
    uint8_t write_buf[2]={reg_add,cmd};
    ret = i2c_master_write_to_device(I2C_MASTER_NUM,i2c_add, write_buf, sizeof(write_buf), I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
    return ret;
}
 ...
 */
static esp_err_t I2C_Read_Reg(unsigned char i2c_add, unsigned char reg_add, unsigned char *data)
{
    return i2c_master_write_read_device(I2C_MASTER_NUM,i2c_add, &reg_add, 1, data,1, I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
}

/**
 * @brief I2C multi read.
 */
static esp_err_t I2C_MultiRead_Reg(unsigned char i2c_add, unsigned char reg_add, int num, unsigned char *data)
{
    return i2c_master_write_read_device(I2C_MASTER_NUM,i2c_add, &reg_add, 1, data,num, I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
}
...
int MMC36X0KJ_Check_OTP(void)
{
	...
	ESP_ERROR_CHECK(I2C_Read_Reg(MMC36X0KJ_7BITI2C_ADDRESS, MMC36X0KJ_REG_STATUS, &reg_val));
...
{
	int ret = 0;
	//float *magnetic=calloc(XYZ,sizeof(float));
